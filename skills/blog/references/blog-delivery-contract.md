...
Every delivered blog ships with these artifacts in `<draft-folder>/`:

### Canonical source of truth (always present)

- `post.json`: canonical article model. The single source of truth from which all output formats are rendered. Contains the full structured article (title, sections, citations, FAQ, schema, metadata). Never edited directly; always generated by prompts.
- `metadata.json`: extracted key fields from the canonical model.
- `schema.json`: standalone JSON-LD schema.org markup.

### Rendered output (one or more, depending on `--format`)

| Format | File | Renderer |
|--------|------|----------|
| markdown (default) | `<slug>.md` | `renderers/markdown.py` |
| mdx | `<slug>.mdx` | `renderers/mdx.py` |
| html | `<slug>.html` | `renderers/generic_html.py` |
| wordpress | `<slug>.wp.html` | `renderers/wordpress_classic.py` |
| wordpress-blocks | `<slug>.blocks.html` | `renderers/wordpress_blocks.py` |
| php | `<slug>.php` | `renderers/php_template.py` |

The rendered output file is always produced from the canonical model. The `.html` and `.pdf` are rendered from the canonical model; they cannot diverge by construction.

### PDF (optional, delivery-contract mode)
...
The legacy markdown-only path (`--md file.md` → `scripts/blog_render.py`) is preserved in full. The canonical path (`--json post.json`) is the recommended path for new content. Both paths produce the same downstream artifacts.

Implementation lives in `scripts/blog_render.py`. Failure to produce the selected format's artifact blocks delivery.
